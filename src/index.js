...
import vertexShader from './shaders/raytrace.vert';
import fragmentShader from './shaders/raytrace.frag';
...
const bodies = centers.map((center, i) => {
  ...
  const colliderDesc = RAPIER.ColliderDesc.ball(radiuses[i] + 0.01);
...
function tick(time) {
  ...
  bodies.forEach((body, i) => {
    ...
    body.applyImpulse(centers[i].clone().multiplyScalar(sign * -0.1));
  })
  controls.update();
